...
public class AssetHunterMainWindow : EditorWindow, ISerializationCallbackReceiver
{
    ...
    private void deleteSelected(List<AssetObjectInfo> objectsToDelete)
    {
        ...
        for (int i = 0; i < objectsToDelete.Count; i++)
        {
			...
			addToGitIgnore(objectsToDelete[i].m_Path);
        }
        UnityEditor.EditorUtility.ClearProgressBar();
    }

	private string gitignoreFilePath = ".gitignore";
	private string gitigoreBaseFile = "base.gitignore";

	private void createFreshGitIgnore()
	{
		File.Copy(gitigoreBaseFile, gitignoreFilePath, true); // this will create a new file or overwrite the old one, good starting point
	}

	private void addToGitIgnore(string pathToAsset)
	{
		using(	StreamWriter sw = File.AppendText (gitignoreFilePath))
		{
			string pathToMetaFile = pathToAsset + ".meta";
			sw.WriteLine(pathToAsset);
			sw.WriteLine(pathToMetaFile);	
		}
	}
